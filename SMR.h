...
static inline void FLAGS_VALID_CK(int option_num, char* option_str[])
{
    const char *flgs[] = { "--bfile","--gwas-summary","--beqtl-summary","--maf","--keep","--remove","--extract-snp","--exclude-snp","--extract-probe","--extract-outcome-probe","--extract-exposure-probe",
        ...
        "--trans-wind","--plot","--eqtl-flist","--smr-format","--merlin-fastassoc-format","--plink-qassoc-format","--gemma-format","--freq","--genes","--smr-wind", "--smr-file","--recode","--probe-var","--chr","--probe-chr","--snp-chr", "--snp", "--from-snp", "--to-snp", "--probe", "--from-probe", "--to-probe","--snp-wind","--probe-wind", "--gene","--from-snp-kb","--to-snp-kb","--from-probe-kb","--to-probe-kb","--extract-single-exposure-probe","--extract-single-outcome-probe","--exclude-single-exposure-probe","--exclude-single-outcome-probe","--gene-list", "--set-list", "--set-wind","--smr-multi","--qfile","--make-besd-dense","--bolt-assoc-format","--geno-uni","--diff","--psmr","--beqtl-qc","--z-thresh","--heidi-mtd","--phet","--count-trans","--count-cis","--opt-selection","--meta","--meta-mtd","--trans","--extract-cis","--rm-technical"
    };
