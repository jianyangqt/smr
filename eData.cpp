...
namespace E_DATA
{
    void read_efile(eData* edata, char* efile)
    {
        int numCol2Skip=2;
        int lineNum=0;
        FILE* eflptr;
        if(!fopen_checked(&eflptr,efile,"r"))
        {
            fprintf (stderr, "%s: Couldn't open file %s\n",
                     efile, strerror (errno));
            exit (EXIT_FAILURE);
        }
        cout << "Reading gene expression / methylation data from [" + string(efile) + "] ..." << endl;
        ...
        fclose(eflptr);
        cout<<"Expression data for "<<edata->prbNum<<" probes of "<<edata->indiNum<<" individuals have been included from the file [" + string(efile) + "]."<<endl;
    }
}
